...
public class Query {
	...
	public static void main(String[] args) {
		...
		LOGGER.info("***********************************");
		LOGGER.info("Welcome to Hadoop non-Indexed Query");
		LOGGER.info("***********************************");
		...
		try {
			...
			LOGGER.error("usage : hadoop jar ${JAR_FILE} "
					...
					+ " <property_file> <column_number> <target_value>");
			System.exit(1);
		...
		try {
			HashMap<String, String> result = index.queryWithoutIndex(
					INDEXED_COLUMN, INPUT_PATH, OUTPUT_PATH, TARGET_VALUE);
			for (Entry<String, String> entry : result.entrySet()) {
				LOGGER.info(entry.getKey() + " : " + entry.getValue());
			}
		} catch (Exception e) {
			LOGGER.error("Test queryWithoutIndex threw exception : "
					+ e.getMessage());
			System.exit(1);
		}
